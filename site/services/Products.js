...
const Product = {
  ...
  addToCart: async function (producto, cantidadPedida, user) {
    ...
    if (productoEnPedidoActivo) {
      await db.Pedido.update(
        {
          cantidad:
            Number(productoEnPedidoActivo.cantidad) + Number(cantidadPedida),
        },
  ...
  cartBuy: async function (user) {
    ...
    const resgistroProducto = pedidosCompra.map(async (pedido) => {
      await db.Producto.update(
        {
          cantidad_real:
            Number(pedido.producto.cantidad_real) - Number(pedido.cantidad),
        },
  ...
  purchaseId: async function (id) {
    console.log("estoy en purchase " + id);
  ...
  viewCompras: async function (user) {
    let pedidosComprados = await db.Pedidos.findAll({
      logging: false,
      where: {
        [Op.and]: [{ user_id: user.id }, { [Op.ne]: { compra_id: null } }],
      },
      include: [
        {
          association: "producto",
