      ...
      const account = {
        total: function () {
          ...
          cart.forEach((product) => {
            total = total + product.producto_id.precio * product.cantidad;
          });
          return total;
        },
        subtotal: () =>
          this.total() - this.total() * (cart.producto_id.iva_id.valor / 100),
        impuestos: () => this.total() * (cart.producto_id.iva_id.valor / 100),
      };
      return res.render("./products/productCart", { cart, account });
    }
    return res.render("./products/productCart", { cart: null, account: null });
  },
